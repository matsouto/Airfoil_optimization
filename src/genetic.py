...
class genetic_algorithm:
    def __init__(
        ...
        self.MAX_CHANGE = (
            MAX_CHANGE  # Variação máxima da posição dos pontos de controle
        )
    ...
    def generate_genome(self):
        ...
        if (self.skip_cp_upper > self.degree_upper // 2) or (
            self.skip_cp_lower > self.degree_lower // 2
        ):
            raise ValueError(
        ...
        for i in range(self.skip_cp_upper):
            genome_upper.extend([self.initial_cp_upper[i]])

        for i in range(self.skip_cp_lower):
            genome_lower.extend([self.initial_cp_lower[i]])

        """Realiza a variação dos parâmetros aleatoriamente"""

        for i in self.initial_cp_upper[
            self.skip_cp_upper : self.degree_upper - self.skip_cp_upper + 1
        ]:
            genome_upper.extend(
                [
                    [
                        i[0] + random.uniform(-self.MAX_CHANGE, self.MAX_CHANGE),
                        i[1] + random.uniform(-self.MAX_CHANGE, self.MAX_CHANGE),
                    ]
                ]
            )

        for i in self.initial_cp_lower[
            self.skip_cp_lower : self.degree_lower - self.skip_cp_lower + 1
        ]:
            genome_lower.extend(
                [
                    [
        ...
        genome.set_name(f"G0N{self.n_genomes}")
        ...
        aux.save_as_dat_from_bezier(
            ...
            genome.cp_lower,
            "generated_airfoil",
            header=f"G0N{self.n_genomes}",
        )
        genome.simulate(
            airfoil_path="airfoils/generated_airfoil.dat",
            name=f"G0N{self.n_genomes}",
    ...
    def plot_population(self, population: list):
        ...
        for k in range(total_subplots):
            ...
            X_upper, Y_upper = aux.generate_bezier_curve(population[k][0])
            X_lower, Y_lower = aux.generate_bezier_curve(population[k][1])
    ...
    def fitness(
        ...
        if genome.converged == True:
            ...
            fitness = (
                fitness_weights[0]
                * (genome.Cl_integration / initial_airfoil.Cl_integration)
                + fitness_weights[1]
                * (initial_airfoil.Cd_integration / genome.Cd_integration)
                + fitness_weights[2]
                * (genome.stall_angle / initial_airfoil.stall_angle)
            )
    ...
    def select_pair(self, population: list):
        ...
        pair += random.choices(
            ...
            weights=[
                self.fitness(genome, self.initial_airfoil, self.fitness_weights) ** 6
                for genome in _population
            ],
            k=1,
        )
    ...
    def crossover(self, parents_genome: list, probability: float):
        ...
        if (len(genome_a.cp_upper) != len(genome_b.cp_upper)) or (
            len(genome_a.cp_lower) != len(genome_b.cp_lower)
        ):
            raise ValueError(
        ...
        if random.random() <= probability:
            ...
            genome_a.set_genome_points(
                a_cp_upper[0:index_upper] + b_cp_upper[index_upper:],
                a_cp_lower[0:index_lower] + b_cp_lower[index_lower:],
            )

            genome_b.set_genome_points(
                b_cp_upper[0:index_upper] + a_cp_upper[index_upper:],
                b_cp_lower[0:index_lower] + a_cp_lower[index_lower:],
            )
    ...
    def mutate(self, genome: bezier_airfoil, probability: float, n_genes: int = 1):
        ...
        if random.random() <= probability:
            ...
            for _ in range(n_genes):
                ...
                cp_upper[index_upper] = [
                    cp_upper[index_upper][0]
                    + random.uniform(-self.MAX_CHANGE, self.MAX_CHANGE),
                    cp_upper[index_upper][1]
                    + random.uniform(-self.MAX_CHANGE, self.MAX_CHANGE),
                ]

                cp_lower[index_lower] = [
                    cp_lower[index_lower][0]
                    + random.uniform(-self.MAX_CHANGE, self.MAX_CHANGE),
                    cp_lower[index_lower][1]
                    + random.uniform(-self.MAX_CHANGE, self.MAX_CHANGE),
                ]
        ...
        for _ in range(generation_limit):
            ...
            for j in range(int(population_size / 2) - 1):
                ...
                aux.save_as_dat_from_bezier(
                    ...
                    header=f"G{self.n_generation}N{self.n_genomes}",
                )
                child_a.simulate(
                    "airfoils/generated_airfoil.dat",
                    "Genome Airfoil",
                    alpha_f=self.alpha_f,
                    alpha_step=self.alpha_step,
                    Re=self.Re,
                ...
                child_a.set_name(f"G{self.n_generation}N{self.n_genomes}")
                ...
                child_b.set_name(f"G{self.n_generation}N{self.n_genomes}")
...
def _example():
    ...
    initial_airfoil.simulate(
        initial_airfoil.airfoil_path,
    ...
    initial_airfoil.get_opt_params(polar_path="src/xfoil_runner/data/initial_polar.txt")
    ...
    gen.run_evolution(fitness_limit=1.1, population_size=3, generation_limit=4)
    ...
    table = tabulate(
        [
            ["Parameter", gen.initial_airfoil.name, gen.final_population[0].name],
            [
                "Cl_Integration",
                gen.initial_airfoil.Cl_integration,
                gen.final_population[0].Cl_integration,
            ],
            [
                "Cd_Integration",
                gen.initial_airfoil.Cd_integration,
                gen.final_population[0].Cd_integration,
            ],
            [
                "Stall_Angle",
                initial_airfoil.stall_angle,
                gen.final_population[0].stall_angle,
            ],
            ["Fitness", 1, gen.final_population[0].fitness],
        ],
        headers="firstrow",
    )
    print(table)
    ...
    table = tabulate(
        [
            ["N_Genomas", "N_Gerações", "N_Crossovers", "N_Mutações"],
            [gen.n_genomes, gen.n_generation, gen.n_crossovers, gen.n_mutation],
        ],
        headers="firstrow",
    )
    print(table)
    print()
    print(f"Fitness Progression: {gen.fitness_progression}")
    print()
    ...
    os.makedirs(f"airfoils/results/{dt_string}")

    for index, genome in enumerate(gen.final_population):
        if index == 5:
            break
        aux.save_as_dat_from_bezier(
            genome.cp_upper,
            genome.cp_lower,
            name=f"results/{dt_string}/genome_{index}",
            header=genome.name,
        )
